#!/bin/sh
set -e
export OAUTH-AUTH-SERVER=$(curl https://$KUBERNETES_SERVICE_HOST}/.well-known/oauth-authorization-server -k|python -c "import sys, json; print(json.load(sys.stdin)['issuer'])")
