#!/bin/sh
set -e
export OAUTH_AUTH_SERVER=$(curl https://$KUBERNETES_SERVICE_HOST}/.well-known/oauth-authorization-server -k|python -c "import sys, json; print(json.load(sys.stdin)['issuer'])")
